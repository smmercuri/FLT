...
OUTPUT_NAME = "flt_build_times.json"  # written under .cache/Pass_<n>/
...
TOOLS = [
    {
        ...
        "description": (
            ...
            "the raw timing data to .cache/Pass_<n>/<stage>/flt_build_times.json "
            "(see the `pass` and `stage` arguments). Each file is built `runs` "
            "times (default 3) and the "
            "wall times averaged to reduce noise. Coarser than `flt_profile_ranking` "
            "(which derives a per-decl own time); use this for whole-file build "
            "cost. NOTE: requires the Mathlib/FLT oleans to be built (e.g. `lake "
            "exe cache get`), else every file errors at the import stage."
        ),
        "inputSchema": {
            "type": "object",
            "properties": {
...
def _cli() -> int:
    ...
    print(_run_timing(None, None, timeout, limit, write_output=True,
                      runs=runs, pass_no=pass_no, stage=stage))
    return 0
