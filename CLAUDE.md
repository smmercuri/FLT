...
| **UPDATE** | Commit the new heartbeats for the changed files to the static tables via `flt_heartbeat_ranking mode=after`. One call recompiles only the changed files plus their direct dependents, updates just those rows in `.cache/metrics_files.json` / `.cache/metrics_decls.json`, writes the `after` snapshot, and freezes a full pass-time copy of both static tables into `.cache/Pass_<n>/after/` (diff these against earlier passes to observe improvement across passes). This is the single point at which the static tables change. Note: this heartbeat remeasure is separate from `TIME_FIX`'s wall-time/instruction payoff timing. | Only the changed rows are updated; static tables current for the next diagnosis. |
| **LOG** | Use the `record_log` tool to record your actions and findings in `.cache/log.jsonl` | -- |
| **STOP** | -- | -- |
